# Proposed 17th amendment — the convergent-inference test (II.22)

**Status: APPLIED — entered as the 17th amendment (II.22).** IV.2 — the
...
amendment — II.22, the convergent-inference test** — plus a proposed
`claims/*.claim.json` subtype for posited, unobserved reference nodes.

The article is entered in `CONSTITUTION.md` (II.22 and amendment-log entry
...
- **II.8 (the difference test)** refuses cheap compatibility — "who a
  ...
  it has no independence vocabulary — the exact axis II.22 adds. The etak is
...
> **II.22 The convergent-inference test.** *Is this knowledge sighted, or is
...
II.22 is a routing precondition on the priors tier, in the same family as
II.2's wall: material knowledge must arrive either with a sighting giver
(II.2) or with a passing convergent-inference case (II.22). Where today a
...
applies II.22 before deciding the wall holds.
...
gap immediately, first run II.22 on the claim's etak structure; a passing
case routes to priors, a failing case is refused citing II.22, and a claim
...
> - **14th — The convergent-inference test (II.22).** Material knowledge
...
offered, both tracing to the same upstream feed — a claim that fails II.22
